...
public class ArmSwinger : MonoBehaviour
{
    ...
    /**
        In case some new guys reading this, with SteamVR 2.0 things like
        SteamVR_ControllerManager, SteamVR_Controller.Device etc. do not exist anymore.
        You have to use "Actions" now.

        Someone postet an update Version: https://paste.ubuntu.com/p/tWfF4CsKBN/
        No compile errors but the swinging does not work 100% I guess, because for me, I even move if no button is pressed, but you get the idea of using actions like
    ...
    **/
    ...
    public SteamVR_Action_Boolean armSwingAction = SteamVR_Input.GetAction<SteamVR_Action_Boolean>("Teleport");
    ...
    //Pablo     public enum ControllerButton {
    //Pablo            Menu,
    //Pablo            Grip,
    //Pablo            TouchPad,
    //Pablo            Trigger
    ...
    private SteamVR_Action_Pose activeSwingController = null;
    ...
    //public GameObject cameraRigGameObject = null;
    //public GameObject Head;
    public Transform CameraRig;
    public Transform Head;
    //CameraRigh and Head not working, assignit it via editor
    //normally in other examples this was used:
    //Head = SteamVR_Render.Top().head;
    //CameraRig = SteamVR_Render.Top().origin;
    //but it gives a NullRefEx in this scene

    //SteamVR Controller Actions
    public SteamVR_Action_Pose RightHandPose;
    public SteamVR_Action_Pose LeftHandPose;
    ...
    public SteamVR_Action_Boolean ButtonYPressed;
    public SteamVR_Action_Boolean StickPresssedDownLeft;
    public SteamVR_Action_Boolean StickPresssedDownRight;
    ...
    void Awake()
    {
        //Warum hier NULL, wenn es in Scene "Locomotion" funktioniert?
        //CameraRig = SteamVR_Render.Top().origin;
        //Head = SteamVR_Render.Top().head;

        // Determine the Steam VR button for ArmSwinging
        //Pablo        steamVRArmSwingButton = convertControllerButtonToSteamVRButton(armSwingButton);
        ...
        previousAngleCheckCameraRigPosition = CameraRig.position;
        lastRaycastHitWhileArmSwinging = raycast(Head.position, Vector3.down, raycastMaxLength, raycastGroundLayerMask);
    ...
    void FixedUpdate()
    {
        ...
        //Debug.Log(leftControllerPreviousLocalPosition);
        //Debug.Log(rightControllerPreviousLocalPosition);


        //???????????????????
        //I always move when pressing the trigger, even without swingig
        //moving direction is the angle of the controller in some way
        ...
        {
            if ((previousCameraRigPushBackPositions.Last.Value != CameraRig.position) ||
                (previousHeadsetPushBackPositions.Last.Value != Head.position))
            {

                savePosition(previousCameraRigPushBackPositions, CameraRig.position, previousPushbackPositionSize);
                savePosition(previousHeadsetPushBackPositions, Head.position, previousPushbackPositionSize);
            }
        }
        ...
        //debugText.text = string.Format("-> {0};{1}", Touchpadposition.axis.x.ToString("N2"), Touchpadposition.axis.y.ToString("N2"));

        //if (ButtonAPressed.state)
        //    debugText.text += "Button A";
    ...
    void verifySettings()
    {
        ...
        // Camera Rig checking
        //Auskommentiert - wohl nicht nötig
        ...
        //Auskommentiert - was mache ich mit dem leftControllerGameObject.layer und rightControllerGameObject.layer
    ...
    Vector3 variableArmSwingMotion()
    {
        ...
        if (movedThisFrame)
        {
            ...
            if (!armSwinging && raycastOnlyHeightAdjustWhileArmSwinging)
            {
                ...
                RaycastHit startSwingingRaycastHit = raycast(Head.position, Vector3.down, raycastMaxLength, raycastGroundLayerMask, out didStartSwingingRayHit);
    ...
    bool swingOneButtonSameControllerExclusive(ref float movement, ref Quaternion rotation)
    {
        ...
        Controller activeController = Controller.None;

        //ist doch dasselbe wie oben
        if (!leftButtonPressed || !rightButtonPressed)
        {
            activeSwingController = null;
        ...
        if (activeSwingController != null)
        {
            ...
            if (activeController == Controller.Left)
            {
                // Find the change in controller position since last Update()
                float leftControllerChange = Vector3.Distance(leftControllerPreviousLocalPosition, leftControllerLocalPosition);
            ...
            else if (activeController == Controller.Right)
            {
                // Find the change in controller position since last Update()
                float rightControllerChange = Vector3.Distance(rightControllerPreviousLocalPosition, rightControllerLocalPosition);
    ...
    void adjustCameraRig()
    {
        ...
        RaycastHit headsetCenterRaycastHit = raycast(Head.position, Vector3.down, raycastMaxLength, raycastGroundLayerMask, out didHeadsetCenterRayHit);
        ...
        if (didHeadsetCenterRayHit)
        {
            ...
            if ((preventClimbing || preventFalling || preventWallWalking) && !outOfBounds)
            {
                ...
                checkAnglesThisFrame = isDistanceFarEnough(previousAngleCheckHeadsetPosition, Head.position, rewindMinDistanceChangeToSavePosition);
                ...
                if (checkAnglesThisFrame && !outOfBounds)
                {
                    ...
                    previousAngleCheckHeadsetPosition = Head.position;
                    previousAngleCheckCameraRigPosition = CameraRig.position;
    ...
    {
        bool didRayHit;
        RaycastHit raycastHit = raycast(Head.position, Vector3.down, raycastMaxLength, raycastGroundLayerMask, out didRayHit);

        if (didRayHit)
        {
            moveCameraRig(new Vector3(CameraRig.position.x, raycastHit.point.y, CameraRig.position.z), PreventionReason.OHAWAS);
            fadeIn();
        }
        else
        {
    ...
    PreventionReason getOutOfBoundsReason(PreventionCheckType checkType, RaycastHit thisHit, RaycastHit lastHit)
    {
        ...
        else if (checkType == PreventionCheckType.WALLWALK)
        {
            ...
            float cameraRigPositionDifferenceY = CameraRig.position.y - previousAngleCheckCameraRigPosition.y;
    ...
    void rewindPosition(PreventionMode preventionMode)
    {
        ...
        Vector3 newCameraRigPosition = calculateCameraRigRewindPosition(cameraRigPreviousPositionToRewindTo, headsetPreviousPositionToRewindTo, CameraRig.position, Head.position, preventionMode);

        CameraRig.position = newCameraRigPosition;
    ...
    void saveRewindPosition(bool force = false)
    {
        ...
        if (force)
        {
            previousCameraRigRewindPositions.AddLast(CameraRig.position);
            previousHeadsetRewindPositions.AddLast(Head.position);
            lastCameraRigRewindPositionSaved = CameraRig.position;
            lastHeadsetRewindPositionSaved = Head.position;
        }
        ...
        if (isDistanceFarEnough(previousSavedPosInWorldUnits, Head.position, rewindMinDistanceChangeToSavePosition) == false)
        {
            return;
        }
        ...
        if (isPositionSafe && preventWallWalking && rewindDontSaveUnsafeWallWalkPositions)
        {
            ...
            if (CameraRig.position.y > previousCameraRigRewindPositions.Last.Value.y)
            {
    ...
    void getControllerButtons()
    {
        ...
        if (armSwingAction.GetState(SteamVR_Input_Sources.RightHand))
        {
        ...
        if (armSwingAction.GetState(SteamVR_Input_Sources.LeftHand))
        {
    ...
    void setupHeadsetCollider()
    {
        ...
        headsetCollider = Head.GetComponent<HeadsetCollider>();
    ...
    void seedRaycastHitHistory()
    {
        ...
        RaycastHit raycastHit = raycast(Head.position, Vector3.down, raycastMaxLength, raycastGroundLayerMask, out didRaycastHit);
    ...
    public bool preventionsPaused
    {
        ...
        set
        {
            ...
            if (value == false)
            {
                moveCameraRig(CameraRig.position);
            }
